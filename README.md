...
| [202](./202-fraud-triage/) | Fraud Triage | **working, live on Rome + Venice** | revenueassurance | LangGraph | Train-serve pipeline, classic ML model as a governed tool, human-approval gate |
| [301](./301-closed-loop-netops/) | Closed-Loop NetOps | **working, live-proven on Rome + Venice** | autonet, agentic | LangGraph + A2A | Multi-agent closed loop, governed actuation (Kuadrant), quant + qual co-decision on the plan |
| [302](./302-energy-optimizer/) | Energy Optimizer | **working, live on Rome + Venice** | airan-energy, sustainability | OGX (Llama Stack) | Simulate-before-act, quant + qual co-decision, LLM-as-a-judge measured |

All five courses run live on the two reference clusters, **Rome** (SNO) and **Venice**; the site's interactive labs replay time-snapshotted Venice captures ([shared/tapes/](./shared/tapes/)).
...
├── 202-fraud-triage/         # working, live on Rome + Venice: pipeline + serving + triage agent
...
├── 301-closed-loop-netops/   # live-proven on Rome + Venice: loop + governed actuation + co-decision
...
│   └── deploy/  QA/  images/ #   as above
├── 302-energy-optimizer/     # live on Rome + Venice: quant + qual co-decision, judge measured
